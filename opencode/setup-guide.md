...
ollama pull exaone3.5:2.4b #(소형 장치 최적화 버전)
ollama pull exaone3.5:7.8b #(기본/가장 많이 쓰이는 버전)
ollama pull exaone3.5:32b #(RTX 5090 환경에서 완전 적재 가능한 고성능 버전)
...
* **`ollama pull deepseek-r1:32b`**
    * **주 용도:** 복잡한 소스코드 디버깅, 수학, 논리 추론, 아키텍처 기획
    * **특징:** 8B 체급과는 차원이 다른 사고 과정(Reasoning)을 시각적으로 보여주며 최적의 정답을 도출합니다.
* **`ollama pull qwen2.5:32b`**
    * **주 용도:** 일반적인 대화, 창작, 범용 AI 비서 업무, 다국어 번역
    * **특징:** 현재 30B 체급 중에서 전 세계 표준에 가까운 완성도와 뛰어난 한국어 지시 이행 능력을 보여줍니다.
* **`ollama pull exaone3.5:32b`**
    * **주 용도:** 한국어 특화 법률/비즈니스 문서 작성, 공문서 요약 및 검토
    * **특징:** LG에서 개발한 국산 최상위 체급 모델로, 한국 고유의 문화적 맥락과 서술체 표현이 가장 정교합니다.
...
* **`ollama pull qwen2.5-coder:32b`** 🔥 *강력 추천*
    * **주 용도:** 실시간 코드 자동완성, 긴 코드 디버깅, 로컬 개발 에이전트 연동
    * **특징:** 기존 7b 체급의 한계를 부수고 고난도 개발 아키텍처를 정확히 이해합니다. Claude 3.5 Sonnet 등 유료 서비스 수준의 코딩 성능을 로컬에서 구현합니다.
...
* **`ollama pull llama3.1:8b`** (또는 최신 `llama3.3:8b`)
    * **주 용도:** 가벼운 질문, 범용 오픈소스 파이프라인 테스트
    * **특징:** 메타(Meta)의 오픈소스 글로벌 표준 모델로 기본기가 가장 탄탄합니다.
* **`ollama pull exaone3.5:7.8b`**
    * **주 용도:** 가벼운 일상 한국어 대화, 빠른 텍스트 필터링 및 분류
    * **특징:** 아까 확인하셨던 올바른 태그 명칭(`7.8b`)으로 작동하는 한국어 고속 경량 모델입니다.
* **`ollama pull exaone3.5:2.4b`**
    * **주 용도:** 소형 엣지 장치 임베딩 연동 및 모바일 백엔드 프로토타이핑
...
* **`ollama pull nomic-embed-text`**
    * **특징:** 오픈소스 RAG에서 가장 대중적으로 널리 쓰이는 가볍고 표준적인 텍스트 임베딩 모델입니다.
* **`ollama pull bge-m3`** 🔥 *추가 추천*
    * **특징:** 다국어(특히 한국어-영어 교차 검색) 환경에서 `nomic`보다 의미론적 검색(Semantic Search) 정확도가 훨씬 뛰어난 고성능 다국어 임베딩 모델입니다.
